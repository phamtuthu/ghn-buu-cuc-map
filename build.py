...
const VN_BOUNDS = [[102.0,7.8],[114.6,23.6]];   // khung Việt Nam gồm Hoàng Sa & Trường Sa
const VN_FIT = { padding: 30 };
const map = new maplibregl.Map({ container:"map", style:STYLES.light, bounds:VN_BOUNDS, fitBoundsOptions:VN_FIT, attributionControl:true });
...
function apply(){
  ...
  renderList();
  if(!p && !q){ if(!firstApply) map.fitBounds(VN_BOUNDS, { ...VN_FIT, duration:600 }); }
  else fitTo(current);
  firstApply=false;
}
